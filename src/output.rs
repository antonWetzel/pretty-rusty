...
impl<'a, Target: OutputTarget> Output<'a, Target> {
	...
	fn emit_indentation(&mut self, state: State, settings: &Settings) {
		match settings.indentation {
			0 => self
				.target
				.emit(&format!("{0:\t<1$}", "", state.indentation), settings),
	...
	pub fn raw(&mut self, text: &str, state: State, settings: &Settings) {
		if text.is_empty() {
			return;
		}
	...
	pub fn finish(mut self, state: State, settings: &Settings) {
		if settings.final_newline {
			self.set_whitespace(Whitespace::LineBreak, Priority::Guaranteed);
		} else {
			self.set_whitespace(Whitespace::None, Priority::Guaranteed);
		}
		self.emit_whitespace(state, settings);
	}
}

// used for aligment W(maybe)IP
