...
pub enum FormatError {
	...
	MalformatedConfigurationFile (#[from]
	toml::de::Error),

	#[error("failed to serialize configuration: {0}")]
	FailedToSerializeConfiguration (#[from]
	toml::ser::Error),
...
pub fn format(command: &Command) -> Result<(), FormatError> {
		...
		let path = match (&command.file_location, &command.path) {
			(Some(path), _) => {
				if path.extension().is_some() {
					path.parent().ok_or(FormatError::FailedToGetProjectFolder)?.to_owned()
				} else {
					path.to_owned()
				}
			}
			(_, Some(path)) => path.to_owned(),
			_ => std::env::current_dir().map_err(FormatError::FailedToGetWorkingDirectory)?.to_owned(),
		};
	...
	let (input_data, input_name) = match (&command.path, command.use_std_in) {
		...
		(Some(path), false) => {
			let input_data = std::fs::read_to_string(path)
				.map_err(FormatError::FailedToReadInputFile)?;
			(input_data, path.display().to_string())
		}
		(None, true) => {
			let mut data = String::new();
			std::io::stdin().read_to_string(&mut data).map_err(FormatError::FailedToReadStdIn)?;
			(data, "stdin".into())
		}
	...
	match (&command.output, command.use_std_out) {
		...
		(None, false) => {
			...
			std::fs::rename(temp_path, input_name).map_err(FormatError::FailedToReplaceInputFile)?;
		}
	};
...
pub fn format_node(
	...
	logic::format_node(&node,  &mut state, &mut output);
	Ok(())
}
