...
pub fn format(command: &Command) -> Result<(), FormatError> {
	...
	match (&command.output, command.use_std_out) { (Some(_), true) => return Err(FormatError::OutputFileAndStdOutSpecified),
	(Some(out), false) => {
...
pub fn format_node(
	...
	logic::format(Node::Node(node), &mut state, settings, &mut output);
